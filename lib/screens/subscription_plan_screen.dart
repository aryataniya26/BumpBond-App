...
class _SubscriptionPlansScreenState extends State<SubscriptionPlansScreen> {
  ...
  void _handlePlanSelection(String planKey) async {
    final currentPlan = await SubscriptionService.getCurrentPlan();

    if (planKey == currentPlan) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('You are already on ${plans[planKey]!['name']} Plan'),
        ),
      );
      return;
    }
    ...
    if (planKey == 'free') {
      await SubscriptionService.setCurrentPlan('free');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  DataRow _buildComparisonRow(String feature, List<String> values) {
    return DataRow(
      cells: [
        ...
        DataCell(_buildComparisonCell(values[0])),
        DataCell(_buildComparisonCell(values[1])),
        DataCell(_buildComparisonCell(values[2])),
        DataCell(_buildComparisonCell(values[3])),
      ],
    );
  }
