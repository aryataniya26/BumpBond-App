...
//   ];
//
//   // 🔹 Function to parse AI or API output
//   List<Map<String, String>> parseSchemes(String content) {
//     try {
...
//           if (item is Map) {
//             return {
//               'scheme': (item['scheme'] ?? item['name'] ?? item['title'] ?? '').toString(),
//               'description': (item['description'] ?? '').toString(),
//               'benefits': (item['benefits'] ?? '').toString(),
//               'howToApply': (item['howToApply'] ?? '').toString(),
//               'link': (item['link'] ?? '').toString(),
//             };
//           }
//           return {'scheme': item.toString()};
//         }).toList();
//       }
//     } catch (_) {
...
//         line = line.trim();
//         if (line.toLowerCase().startsWith('scheme:')) {
//           map['scheme'] = line.substring(7).trim();
//         } else if (line.toLowerCase().startsWith('description:')) {
//           map['description'] = line.substring(12).trim();
//         } else if (line.toLowerCase().startsWith('benefits:')) {
//           map['benefits'] = line.substring(9).trim();
//         } else if (line.toLowerCase().startsWith('how to apply:')) {
//           map['howToApply'] = line.substring(13).trim();
//         } else if (line.toLowerCase().contains('http')) {
//           map['link'] = RegExp(r'(https?:\/\/[^\s]+)').firstMatch(line)?.group(0) ?? '';
//         }
//       }
//
//       if (map['scheme']!.isNotEmpty) results.add(map);
//     }
//
//     return results.isEmpty
//         ? [
//       {
//         'scheme': 'No schemes found',
//         'description': 'No data available for this state.',
//         'benefits': '-',
//         'howToApply': '-',
//         'link': '-'
//       }
...
//   }
//
//   // 🔹 Fetch schemes dynamically using OpenAI or fallback
//   Future<void> fetchPolicies(String state) async {
//     setState(() {
//       isLoading = true;
//       policies = [];
...
//
//     try {
//       // Replace this endpoint + key with your own AI or backend API
//       final response = await http.post(
//         Uri.parse('https://api.openai.com/v1/chat/completions'),
//         headers: {
//           'Content-Type': 'application/json',
//           'Authorization': 'Bearer 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', // <-- Add your key
//         },
//         body: jsonEncode({
//           'model': 'gpt-4o-mini',
//           'messages': [
...
//               'role': 'user',
//               'content':
//               'List 3 government maternity and pregnancy benefit schemes available in $state, India. '
//                   'Include for each: scheme name, short description, benefits, how to apply, and official link. '
...
//         final parsed = parseSchemes(content);
//         setState(() => policies = parsed);
//       } else {
...
//             {
//               'scheme': 'Janani Suraksha Yojana (JSY)',
//               'description': 'Encourages institutional delivery among poor pregnant women.',
//               'benefits': '₹700–₹1000 cash incentive.',
//               'howToApply': 'Apply through ASHA/Anganwadi Centre.',
//               'link': 'https://www.nhp.gov.in/janani-suraksha-yojana_pg',
//             },
//             {
//               'scheme': 'Pradhan Mantri Matru Vandana Yojana (PMMVY)',
//               'description': '₹5000 maternity benefit for first live birth.',
//               'benefits': 'Installment payments via Direct Benefit Transfer.',
//               'howToApply': 'Apply via Anganwadi or pmmvy.gov.in.',
...
//                     }).toList(),
//                     onChanged: (String? newState) {
//                       if (newState != null && newState != 'Select a State') {
//                         setState(() => selectedState = newState);
//                         fetchPolicies(newState);
//                       }
//                     },
...
//             else if (policies.isEmpty)
//               const Text("Please select a state to view government schemes.",
//                   style: TextStyle(color: Colors.grey))
//             else
//               Column(
...
//                                     color: Colors.green, fontSize: 13)),
//                             const SizedBox(height: 6),
//                             Text("How to Apply: ${p['howToApply'] ?? '-'}",
//                                 style: const TextStyle(
//                                     color: Colors.blueGrey, fontSize: 13)),
//                             const SizedBox(height: 10),
//                             ElevatedButton.icon(
