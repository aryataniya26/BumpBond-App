...
class _MedicationRemindersScreenState extends State<MedicationRemindersScreen> {
  ...
  void _showEditMedicationScreen(QueryDocumentSnapshot medication) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AddMedicationScreen(
        ...
        children: [
          Row(
            children: [
              ...
              // Edit Button
              IconButton(
                icon: const Icon(Icons.edit_outlined, color: Color(0xFFB794F4)),
                onPressed: () => _showEditMedicationScreen(med),
              ),
              // Delete Button
              IconButton(
...
class AddMedicationScreen extends StatefulWidget {
  ...
  final QueryDocumentSnapshot? existingMedication;
...
class _AddMedicationScreenState extends State<AddMedicationScreen> {
  ...
  void _loadExistingData() {
    if (widget.existingMedication != null) {
      final data = widget.existingMedication!.data() as Map<String, dynamic>;

      _nameController.text = data['name'] ?? '';
      _dosageController.text = data['dosage'] ?? '';
      _selectedFrequency = data['frequency'] ?? '';
      _selectedTimeSlots = List<String>.from(data['timeSlots'] ?? []);
      _purposeController.text = data['purpose'] ?? '';
      _prescribedByController.text = data['prescribedBy'] ?? '';
  ...
  void _saveMedication() async {
    ...
    try {
      ...
      // Add createdAt only for new medications
      if (widget.existingMedication == null) {
      ...
      if (widget.existingMedication != null) {
        ...
        await widget.medicationCollection
            .doc(medicationId)
            .update(medicationData);
        print('✅ Medication updated with ID: $medicationId');
      } else {
        // Add new medication
        final docRef = await widget.medicationCollection.add(medicationData);
        medicationId = docRef.id;
        print('✅ Medication saved with ID: $medicationId');

        // Try to schedule notifications (but don't fail if it doesn't work)
        try {
          await _scheduleNotifications(medicationId);
        } catch (notificationError) {
          print('⚠️ Notification scheduling failed: $notificationError');
          // Don't fail the entire save if notifications fail
        }
      }

      // ✅ SUCCESS - Reset form and navigate
      _handleSuccess(medicationId);
      ...
      _showSnackbar('Error saving medication', isError: true);
    }
  }

  // New method to handle notification scheduling
  Future<void> _scheduleNotifications(String medicationId) async {
    for (String slot in _selectedTimeSlots) {
      try {
        await _scheduleNotification(slot, medicationId);
      } catch (e) {
        print('❌ Failed to schedule notification for $slot: $e');
        // Continue with other time slots even if one fails
      }
    }
  }

  // Updated notification scheduling with error handling
  Future<void> _scheduleNotification(String timeSlot, String medicationId) async {
    try {
      ...
      if (notificationTime != null) {
        ...
        // Use inexact scheduling to avoid permission issues
        await widget.flutterLocalNotificationsPlugin.zonedSchedule(
          ...
          scheduledTime,
          notificationDetails,
          androidScheduleMode: AndroidScheduleMode.inexactAllowWhileIdle, // Changed to avoid exact alarm error
          matchDateTimeComponents: DateTimeComponents.time,
        );

        print('✅ Notification scheduled for $timeSlot at ${notificationTime.hour}:${notificationTime.minute}');
      }
    } catch (e) {
      print('❌ Notification scheduling error for $timeSlot: $e');
      // Don't rethrow - let the medication save continue
    }
  }

  // Handle success after saving
  void _handleSuccess(String medicationId) {
    final isEditing = widget.existingMedication != null;

    // Call the callback to refresh the list
    widget.onMedicationAdded?.call();

    if (isEditing) {
      // For editing, just go back
      Navigator.pop(context);
      _showSnackbar('Medication updated successfully!', isError: false);
    } else {
      // For new medication, reset form and show success
      _resetForm();
      _showSnackbar('Medication added successfully!', isError: false);
    }
  }

  // Method to reset the form completely
  void _resetForm() {
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              isEditing ? 'Update medication details' : 'Enter medication details',
              style: const TextStyle(
                color: Colors.white70,
                fontSize: 12,
              ),
...
//         print('✅ Medication saved with ID: $medicationId');
//
//         // Schedule notifications only for new medications (with error handling)
//         try {
//           await _scheduleNotifications(medicationId);
//         } catch (notificationError) {
//           print('⚠️ Notification scheduling failed: $notificationError');
//           // Don't fail the entire save if notifications fail
//           _showSnackbar(
//             'Medication saved but notifications not set up',
//             isError: false,
//           );
//         }
//       }
//
//       // ✅ SUCCESS - Reset form and navigate
//       _resetFormAndNavigate(medicationId);
//
//     } catch (e) {
//       // ✅ ERROR HANDLING
//       setState(() {
...
//       });
//       print('❌ Error saving medication: $e');
//       _showSnackbar('Error saving medication: ${e.toString()}', isError: true);
//     }
//   }
//
// // New method to handle notification scheduling with better error handling
//   Future<void> _scheduleNotifications(String medicationId) async {
//     for (String slot in _selectedTimeSlots) {
//       try {
//         await _scheduleNotification(slot, medicationId);
...
//   }
//
// // Updated notification scheduling with better error handling
//   Future<void> _scheduleNotification(String timeSlot, String medicationId) async {
//     try {
//       TimeOfDay? notificationTime;
//
...
//   }
//
// // Updated method to reset form and navigate
//   void _resetFormAndNavigate(String medicationId) {
//     // Call the callback to refresh the list
//     widget.onMedicationAdded?.call();
//
//     final isEditing = widget.existingMedication != null;
...
//       _showSnackbar('Medication updated successfully!', isError: false);
//     } else {
//       // For new medication, show success and reset form
//       _resetForm();
//       _showSnackbar('Medication added successfully!', isError: false);
//
//       // Optionally, you can choose to go back or stay on the form
//       // Uncomment the next line if you want to go back to list after adding
//       // Navigator.pop(context);
//     }
//   }
//
...
//     });
//
//     // Show success message for adding another
//     _showSnackbar(
//         'Form cleared! You can add another medication.',
//         isError: false
//     );
//   }
//
